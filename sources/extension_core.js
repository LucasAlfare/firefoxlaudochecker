/*
 *****************************************
 * CARREGA MODAL DE ALERTA PERSONALIZADO *
 * ***************************************
 */
...
/*
 *****************************************
 *    NÚCLEO DAS CHECAGENS DA EXTENSÃO   *
 * ***************************************
 */
...
window.addEventListener('load', function () {
    ...
    const nums = laudoInfo.replace(/\./g, "").replace(/,/g, ".").match(/[\d.]+/g);
    ...
    if (nums.length === 98) {
        ...
        if (getItemInfo('leucocitos').resultadoInterp === -1) {
            acoes.push("Leucopenia. Sugere-se correlação clínica");
        } else if (getItemInfo('leucocitos').resultadoInterp === 1) {
            acoes.push("Leucocitose. . Sugere-se correlação clínica");
        }
        ...
        if (aux.resultadoInterp === -1) {
            if (aux.resultadoItem < 100000) {
                ...
                swal("Não libere!", "Plaquetas muito baixas.", "error");
                return;
            } else if (aux.resultadoItem > (aux.min - 2000)) {
                acoes.push("altere o valor das plaquetas para 150000");
                swal("Atenção:", "altere o valor das plaquetas para 150000.", "warning")
            } else {
        ...
        if (acoes.length !== 0) {
            ...
            swal({
                title: "Alterações para adicionar:",
                text: s,
                icon: "info"
            });
        }
    } else {
        swal("ERRO!", "Laudo acessado está diferente do padrão.", "error");
    }
});
